...
class ConfigCog(commands.Cog, name="Configuration"):
	...
	async def rules(self, ctx):
		...
		buf = "How to play:\n"
		buf += "- Draft a team of valorant players, and compete to see whose players have the best performance over the course of the event\n"
		...
		buf += "- Each team can only have ONE player from a given pro team in active slots (IGL through Sentinel)\n"
		buf += "- Each team has 6 active slots and " + str(self.bot.sub_slots) + " sub slot(s)\n"
		buf += "- The Flex slot bypasses the one-player-per-team restriction\n"
		buf += "- Only players in active slots count towards your team's total points\n"
		buf += "- At the end of the season, the fantasy team with the best record wins\n"
		buf += "\n"
		buf += "Draft phase:\n"
		buf += "- There will be " + str(self.bot.num_rounds) + " rounds\n"
		buf += "- Snake draft (1234554321123...)\n"
		buf += "- The draft is asynchronous — you will be pinged when it is your turn\n"
